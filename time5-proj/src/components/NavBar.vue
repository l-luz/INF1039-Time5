<template>
    <v-layout>
        ...
        <v-app-bar color="#000000">
                ...
                <v-spacer />
                <v-text-field class="h-75 py-1" single-line  density="compact" prepend-inner-icon="mdi-magnify" min-width="400px" placeholder="Buscar" variant="outlined" />
                <v-spacer />
                <v-spacer />
                <v-btn variant="outlined" :to="{ name: 'login' }" text="Login" />
                <v-btn class="pink-button" :to="{name: 'signup'}" text="Register" />
        </v-app-bar>
